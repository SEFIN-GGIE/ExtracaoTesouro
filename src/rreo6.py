...
df_municipio = pd.read_excel('dicio/cd_municipio_.xlsx', sheet_name='Sheet1', usecols=['cod_completo', 'nome_municipio'])
...
for ano in range(2018, 2024):
    for cd_municipio, municipio in zip(cd_mun, mun):
        for bimestre in [1, 2, 3, 4, 5, 6]:
            ...
            url = f"https://apidatalake.tesouro.gov.br/ords/siconfi/tt/rreo?an_exercicio={ano}" \
...
dataframes.to_csv(f"../processed/rreo/rreo_3_capitais_2015_2023.csv", sep=";", index=False, decimal=",")
