...
def main():
    ...
    with st.sidebar:
        ...
        periodo_sel = st.multiselect(
            "Selecione o período de referência (bimestre/quadrimestre)",
            options=range(1, 7),
        )
        mun_sel = st.multiselect(
            "Selecione o ente", df_municipio.nome_municipio.unique()
        )
        # esfera_sel = st.selectbox("Selecione a esfera", options=['M', 'E', 'U', 'C'])
        anexo_sel = st.selectbox(
            "Selecione o anexo",
            options=["01", "02", "03", "04", "05", "06", "07", "10"],
        )

    # Aplicando filtros de entes
    df_municipio = df_municipio[df_municipio["nome_municipio"].isin(mun_sel)]
    cd_mun_sel, mun_sel = (
        df_municipio.cod_completo.to_list(),
        df_municipio.nome_municipio.to_list(),
    )
